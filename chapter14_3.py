...
"""사목, 작성자 : ****
입체 사목과 유사한, 타일을 떨어뜨려서 4개를 한 줄로 늘어놓는 게임"""
...
def getPlayerMove(playerTile, board):
    ...
    while True: # 플레이어가 유효한 움직임 명령을 입력할 때까지 계속 요청한다.
        ...
        columnIndex = int(response)-1 # 0기반 인덱스

        # 가득 찬 열이면, 움직임 명령을 다시 요구한다.
...
def isFull(board):
    """ board에 빈칸이 없으면 True를 반환한다.
    그렇지 않으면 False를 반환한다."""
    for rowIndex in range(BOARD_HEIGHT):
        for columnIndex in range(BOARD_WIDTH):
            if board[(columnIndex, rowIndex)] == EMPTY_SPACE:
                return False # 빈칸을 발견했으므로 False를 반환
    return True

def isWinner(playerTile, board):
    """playertile이 board에서 사목을 완성하면 True를 반환하고,
    그렇지 않으면 False를 반환한다."""
